...
* **📷/🖼️/✏️ 3가지 모바일 복약 추가 방식 연동**:
  * 처방전 추가 옵션 모달 창에 **"🖼️ 앨범에서 사진 불러오기"** 옵션을 신규 탑재했습니다. 이를 통해 카메라 촬영뿐 아니라 사용자가 이미 찍어둔 갤러리 이미지첩에서 처방전 스캔본을 선택해 Gemini AI OCR에 전송하고 일괄 분석을 받아 볼 수 있습니다.
* **복약 기록 추가 방식 선택 모달 및 수동 직접 입력 기능 탑재**:
  * 메인 추가 버튼을 `새 복약 기록 추가`로 개편하고, 터치 시 **"📷 카메라로 처방전 스캔"**, **"🖼️ 앨범에서 사진 불러오기"**, **"✏️ 직접 입력하여 추가"** 중 환자가 편리한 수단을 고를 수 있는 옵션 시트를 제공합니다.
...
### 11. 안드로이드 시스템 네비게이션 영역 중첩 방지(SafeArea) 개선 (2026-07-30)
* **📱 새 건강 기록 추가 옵션 시트 네비게이션바 간섭 방지**:
  * 메인 화면의 '새 건강 기록 추가' 버튼을 눌렀을 때 나타나는 추가 수단 선택 바텀 시트(`showModalBottomSheet`)에 `useSafeArea: true` 속성을 명시적으로 부여하고 하단 패딩을 조정하여, 소프트웨어 네비게이션 바(소프트키) 또는 제스처 바와 카드 터치 영역이 겹치는 현상을 완전히 제거했습니다.
* **🔑 바이탈패스 QR 코드 화면 취소 버튼 안전 영역 적용**:
  * 환자가 의료진에게 QR 코드를 전달하는 `QrGeneratorScreen` 화면에 `SafeArea` 레이아웃 래퍼를 도입하여, 최하단에 위치한 빨간색 '취소' 아웃라인 버튼이 안드로이드 OS 하단 기능 버튼과 겹치는 버그를 완벽히 해결했습니다.

### 12. 글자 크기 조절 옵션 문구 순화 및 접근성 고도화 (2026-07-30)
* **🔎 특정 단어(어르신 배려 등) 순화 처리**:
  * 글자 크기 변경 옵션명 및 라벨 텍스트에서 '어르신 배려', '어르신 추천' 등의 지칭 단어를 제외하여 모든 시력 불편 사용자에게 보편적인 사용성을 주도록 문구를 조정했습니다:
    * `"화면 글자 크기 조절 (어르신 배려)"` ➔ `"화면 글자 크기 조절"`
    * `'큰 글씨 (1.30배 어르신 추천)'` ➔ `'큰 글씨 (1.30배 추천)'`
